...
- 前端：Vue 3、tdesign-vue-next、Pinia、Vue Router、Vite；doocs/md 编辑器核心（集成方式见 research.md）
...
| V. 开源协作与透明 | 许可证兼容、无强制遥测 | doocs/md 许可证须在集成前核验（research.md 决策点）；无遥测 | NEEDS-VERIFY |

**门禁结论**：通过。唯一需在实现前关闭的事项是核验 doocs/md 许可证与本项目开源许可证的兼容性
（已列为 Phase 0 研究项与后续 P1 任务的前置）。无违反原则需进入 Complexity Tracking。
